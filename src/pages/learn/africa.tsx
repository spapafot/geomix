...
export async function getStaticProps() {
  const file = path.join(process.cwd(), "public/data/africa.geojson");
...
export default function LearnAfricaPage({ countries }: Props) {
  const { t, i18n } = useTranslation();
  const isEn = i18n.language === "en";
  const k = "pages.africa";
      ...
      <div className="min-h-screen bg-slate-950 text-white">
        ...
        <main className="max-w-3xl mx-auto px-6 py-16">
          <nav className="text-xs text-slate-500 mb-8 flex items-center gap-2">
            ...
            <span>›</span>
            <span className="text-slate-400">{t("learn.articles.africa.title")}</span>
          </nav>

          <div className="flex items-center gap-3 mb-6">
            <span className="text-4xl">🌍</span>
            <h1 className="text-4xl font-extrabold text-slate-100">{t("learn.articles.africa.title")}</h1>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-6">
            ...
            <div className="flex flex-wrap gap-3">
              <Link href="/quiz/africa" className="px-4 py-2 bg-amber-600 hover:bg-amber-500 rounded-lg text-sm font-semibold transition-colors">
                🗺️ {t("learn.articles.africa.title")}
              </Link>
