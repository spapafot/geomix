...
export async function getStaticProps() {
  const file = path.join(process.cwd(), "public/data/asia.geojson");
...
export default function LearnAsiaPage({ countries }: Props) {
      ...
      <div className="min-h-screen bg-slate-950 text-white">
        ...
        <main className="max-w-3xl mx-auto px-6 py-16">
          <nav className="text-xs text-slate-500 mb-8 flex items-center gap-2">
            ...
            <span>›</span>
            <span className="text-slate-400">{t("learn.articles.asia.title")}</span>
          </nav>

          <div className="flex items-center gap-3 mb-6">
            <span className="text-4xl">🌏</span>
            <h1 className="text-4xl font-extrabold text-slate-100">{t("learn.articles.asia.title")}</h1>
          </div>
